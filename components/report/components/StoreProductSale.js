...
import { QUERY_SALESTOREPRODUCTDAILY } from '../../../apollo/query';
...
const ProductSale = () => {
  ...
  const { data, loading } = useQuery(QUERY_SALESTOREPRODUCTDAILY, {
    variables: {
    ...
    onCompleted: (data) => {
      setStoreProductSaleData(data.saleStoreProductDaily);
    },
  });
      ...
      >
        <CardContent>
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <Typography>ยอดขายสินค้าในร้านประจำวัน</Typography>
          </div>
        </CardContent>
        <div>
          {loading ? (
            ...
            storeProductSaleData && (
              <MotionSlider
                ...
                allowSlideToLast
              >
                {storeProductSaleData?.map((product) => (
                  <div key={product.id}>
                    <div
                      style={{
                        display: 'flex',
