...
import { MUTAION_DELETE_STOCKCATALOG } from '../../apollo/mutation';

// Toast
import { useToasts } from 'react-toast-notifications';

const DeleteStockCatalog = ({ setRerender }) => {
  const catalogs = useSelector((state) => state.stock.stockCatalog);
  const matches600down = useMediaQuery('(max-width:600px)');
  const action = useDispatch();
  const theme = useTheme();
  const { addToast } = useToasts();
  const [deleteStockCatalog, { loading, error }] = useMutation(
    ...
    >
      ...
      >
        <DialogTitle style={{ cursor: 'move' }} id="draggable-dialog-title">
          <Typography align="center">ต้องการลบประเภทสินค้า</Typography>
...
export default DeleteStockCatalog;
